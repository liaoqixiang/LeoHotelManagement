...
VALUES (
	...
	,@uRemark
            ...
            var res = SqlHelper.ExecuteScalar(sql,
					...
					new SqlParameter("@uRemark", model.uRemark)					
                );
        ...
        {
            const string sql = "DELETE FROM [dbo].[Admin] WHERE [uId] = @uId";
...
WHERE [uId] = @uId";
        ...
        public IEnumerable<Admin> QueryList(int index, int size, object wheres, string orderField, bool isDesc = true)
        {
            ...
            var sql = SqlHelper.GenerateQuerySql("Admin", new[] { "uId", "uPwd", "uName", "uIsDel", "uRemark" }, index, size, whereBuilder.ToString(), orderField, isDesc);
            using (var reader = SqlHelper.ExecuteReader(sql, parameters.ToArray()))
            {
                if (reader.HasRows)
                {
                    while (reader.Read())
                    {
                        yield return SqlHelper.MapEntity<Admin>(reader);
                    }
                }
            }
        }
        ...
        {
            const string sql = "SELECT TOP 1 [uId], [uPwd], [uName], [uIsDel], [uRemark] FROM [dbo].[Admin] WHERE [uId] = @uId";
            using (var reader = SqlHelper.ExecuteReader(sql, new SqlParameter("@uId", uid)))
            {
                if (reader.HasRows)
                {
                    reader.Read();
                    return SqlHelper.MapEntity<Admin>(reader);
                }
                else
                {
                    return null;
        ...
        public int QueryCount(object wheres)
        {
            ...
            var sql = SqlHelper.GenerateQuerySql("Admin", new[] { "COUNT(1)" }, whereBuilder.ToString(), string.Empty);
